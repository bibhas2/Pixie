...
- (id) initWithUniqueId: (const char*) uniqueId requestRecord: (RequestRecord*) req responseRecord: (ResponseRecord*) res{
    ...
    if (self != nil) {
        self.uniqueId = [NSString stringWithUTF8String:uniqueId];
        self.method = [NSString stringWithUTF8String:stringAsCString(req->method)];
        self.host = [NSString stringWithUTF8String:stringAsCString(req->host)];
        self.statusMessage = res->statusMessage->length == 0 ? @"In progress" :
            [NSString stringWithUTF8String:stringAsCString(res->statusMessage)];
        self.statusCode = [NSString stringWithUTF8String:stringAsCString(res->statusCode)];
        //Strip out extra stuff from path
        size_t i = 0;
        for (i = 0; i < req->path->length; ++i) {
            if (req->path->buffer[i] == ' ' || req->path->buffer[i] == '?') {
